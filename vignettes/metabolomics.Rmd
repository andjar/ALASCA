...
#mod <- ALASCA(df, form, separateTimeAndGroup = FALSE, forceEqualBaseline = TRUE, scaleFun = scaleFun, method = "LMM", useRfast = FALSE)


devtools::load_all()
mod <- ALASCA(df, form, separate_time_and_group = FALSE, equal_baseline = TRUE, scale_function = "sdall", validate = TRUE, n_validation_runs = 10)
```
...
We can also assess the pair-wise differences for each variable;

```{r}
assessGroupDifferences(mod)
```
